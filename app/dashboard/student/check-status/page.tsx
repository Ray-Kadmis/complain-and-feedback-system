...
export default function CheckStatus() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        await fetchActiveComplaints(user.uid)
      } else {
  ...
  const fetchActiveComplaints = async (userId: string) => {
    try {
      // First, try a simpler query without orderBy to avoid index issues
      const q = query(
        collection(db, "complaints"),
        where("userId", "==", userId),
        where("status", "in", ["pending", "in-progress", "under review", "active", "awaiting confirmation"]),
      )
    ...
    <div className="container mx-auto py-10">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Check the status of your active complaints</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
