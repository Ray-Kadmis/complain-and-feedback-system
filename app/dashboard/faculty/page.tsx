...
export default function FacultyDashboard() {
  ...
  const [awaitingConfirmationComplaints, setAwaitingConfirmationComplaints] = useState<Complaint[]>([])
  ...
  const fetchComplaints = async (userId: string) => {
    try {
      ...
      const q = query(collection(db, "complaints"), where("assignedTo", "==", userId))
      ...
      awaitingConfirmation.sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime())
  ...
  const handleStatusChange = async (complaintId: string, newStatus: string) => {
    try {
      ...
      if (newStatus === "active") {
        ...
        if (complaint) {
          const updatedComplaint = { ...complaint, status: newStatus, updatedAt: new Date() }
          setReceivedComplaints(receivedComplaints.filter((c) => c.id !== complaintId))
        ...
        if (complaint) {
          ...
          setAwaitingConfirmationComplaints([updatedComplaint, ...awaitingConfirmationComplaints])
        }
      }

      toast.success("Status updated", {
        description: `Complaint status has been updated to ${newStatus.replace(/-/g, " ")}.`,
      })
    } catch (error) {
      console.error("Error updating complaint status:", error)
      toast.error("Error", {
        description: "There was a problem updating the complaint status. Please try again.",
      })
    }
  }
  ...
  const toggleExpand = (
    ...
    setComplaintsList(complaintsList.map((c) => (c.id === id ? { ...c, isExpanded: !c.isExpanded } : c)))
  }
            ...
            <CardContent className="pt-6">
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {showActionButton && (
                    <Button size="sm" variant="outline" onClick={() => onAction(complaint.id)}>
                      {actionButtonText}
                    </Button>
                  )}
                  {complaint.status === "resolved" && complaint.studentResolutionResponse && (
                    <Badge variant={complaint.studentResolutionResponse === "confirmed" ? "success" : "destructive"}>
                      {complaint.studentResolutionResponse === "confirmed" ? "Student Confirmed" : "Student Rejected"}
                    </Badge>
                  )}
                </div>
              </div>
              <div className="mt-2 w-full">
                <div className="flex w-full">
                  <Button
                    ...
                    onClick={() => toggleExpand(complaints, setComplaints, complaint.id)}
                  >
                    <ChevronDown
                      className={`h-4 w-4 transition-transform ${complaint.isExpanded ? "transform rotate-180" : ""}`}
                    />
                  </Button>
      ...
      <Tabs defaultValue="received" className="max-w-4xl mx-auto">
        ...
        <TabsContent value="received">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Complaints that have been forwarded to you for review</CardDescription>
            </CardHeader>
            <CardContent>
              {renderComplaintsList(receivedComplaints, setReceivedComplaints, true, "Mark as Active", (id) =>
        ...
        <TabsContent value="active">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Complaints you are currently working on</CardDescription>
            </CardHeader>
            <CardContent>
              {renderComplaintsList(activeComplaints, setActiveComplaints, true, "Mark as Resolved", (id) =>
                handleStatusChange(id, "awaiting confirmation"),
              )}
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="awaiting">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Complaints waiting for student confirmation</CardDescription>
            </CardHeader>
            <CardContent>
              {renderComplaintsList(awaitingConfirmationComplaints, setAwaitingConfirmationComplaints)}
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="resolved">
          <Card>
            ...
            <CardContent>{renderComplaintsList(resolvedComplaints, setResolvedComplaints)}</CardContent>
          </Card>
        </TabsContent>
      </Tabs>
    </div>
